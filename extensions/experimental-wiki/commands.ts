...
export function registerWikiCommands(pi: ExtensionAPI): void {
  pi.registerCommand("fleet:wiki:status", {
    description: "Fleet Wiki 저장소 상태 확인 및 필요 시 초기화",
  ...
  pi.registerCommand("fleet:wiki:queue", {
    description: "Fleet Wiki patch queue 목록 표시",
    handler: async (_args, ctx) => {
      const items = await listQueue(resolveMemoryPaths(ctx.cwd));
      if (items.length === 0) {
        ctx.ui.notify("Queue is empty. Use Fleet Wiki capture staging to create a pending patch.", "info");
        return;
      }
      const ids = items.map((item) => item.id).join(", ");
      ctx.ui.notify(`Queue items (${items.length}): ${ids}. Next: /fleet:wiki:show <patch_id>`, "info");
    },
  });

  pi.registerCommand("fleet:wiki:show", {
    description: "Fleet Wiki patch queue 항목 조회",
  ...
  pi.registerCommand("fleet:wiki:approve", {
    description: "Fleet Wiki patch 승인",
    handler: async (args, ctx) => {
      const patchId = args.trim();
      if (!patchId) {
        const items = await listQueue(resolveMemoryPaths(ctx.cwd));
        const suffix = items.length > 0 ? ` Available patch IDs: ${items.map((item) => item.id).join(", ")}` : " Queue is empty.";
        ctx.ui.notify(`/fleet:wiki:approve <patch_id> is required.${suffix}`, "warning");
        return;
      }
      try {
  ...
  pi.registerCommand("fleet:wiki:reject", {
    description: "Fleet Wiki patch 반려",
    handler: async (args, ctx) => {
      const trimmed = args.trim();
      if (!trimmed) {
        const items = await listQueue(resolveMemoryPaths(ctx.cwd));
        const suffix = items.length > 0 ? ` Available patch IDs: ${items.map((item) => item.id).join(", ")}` : " Queue is empty.";
        ctx.ui.notify(`/fleet:wiki:reject <patch_id> [reason] is required.${suffix}`, "warning");
        return;
      }
  ...
  pi.registerCommand("fleet:wiki:drydock", {
    description: "Fleet Wiki 정적 점검 실행",
    handler: async (_args, ctx) => {
  ...
  pi.registerCommand("fleet:wiki:capture", {
    description: "현재 세션 이력에서 Fleet Wiki 캡처 staging 또는 preview 후속 흐름 시작",
    handler: async (_args, ctx) => {
      ...
      if (typeof pi.sendUserMessage !== "function") {
        throw new Error("fleet:wiki:capture requires PI sendUserMessage support");
      }
