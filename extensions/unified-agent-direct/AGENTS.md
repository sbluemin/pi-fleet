...
| File | Role |
|------|------|
| `index.ts` | Entry point: Wiring only — initialization, registration calls, session events |
...
| `core/panel/widget-sync.ts` | PI TUI widget bridge (syncWidget, syncFooterStatus) |
| `core/panel/config.ts` | Model/service config setters + height adjustment |
...
| `core/render/panel-renderer.ts` | Agent panel rendering (full/compact/banner views) |
...
| **tools/** | |
| `tools/index.ts` | Registers `claude`, `codex`, `gemini` as individual pi tools |
| `tools/prompts.ts` | Tool descriptions, prompt snippets, guidelines, cross-report prompt |
| **model-selection/** | |
| `model-selection/model-ui.ts` | Model selection UI + keybind/command registration |
| **status/** | |
| `status/` | Service status monitoring (Claude/Codex/Gemini health checks) |
