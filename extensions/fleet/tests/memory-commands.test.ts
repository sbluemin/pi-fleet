...
import { enqueuePatch, parsePatch } from "../memory/patch.js";
...
describe("memory commands", () => {
  it("shows queue selection guidance as a warning instead of throwing for unknown show IDs", async () => {
    ...
    const { config } = registerCommand("fleet:memory:show");
    const notify = vi.fn();
  ...
  it("shows queue selection guidance as a warning instead of throwing for unknown approve/reject IDs", async () => {
    ...
    const approve = registerCommand("fleet:memory:approve").config;
    const reject = registerCommand("fleet:memory:reject").config;
...
function registerCommand(name: string): { config: any } {
  ...
  registerMemoryCommands({ registerCommand, sendUserMessage: vi.fn() } as any);
...
async function makeTempRoot(): Promise<string> {
  const root = await mkdtemp(path.join(os.tmpdir(), "fleet-memory-commands-"));
