...
import { runDryDock } from "../memory/drydock.js";
import { PATCH_FILENAME } from "../memory/constants.js";
import { resolveMemoryPaths } from "../memory/paths.js";
import { writeRawSourceEntry, writeWikiEntry } from "../memory/store.js";
...
describe("memory drydock", () => {
  ...
  it("flags prompt-injection-like memory content", async () => {
    const root = await makeTempRoot();
    const paths = resolveMemoryPaths(root);
...
async function makeTempRoot(): Promise<string> {
  const root = await mkdtemp(path.join(os.tmpdir(), "fleet-memory-drydock-"));
