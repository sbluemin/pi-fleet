...
- **Carrier Result Push**: Follow-up pushes use `pi.sendMessage` custom messages with `customType: "carrier-result"`, `display: false`, and a `<system-reminder>`-wrapped `[carrier:result]` payload. Framework push delivery must never be sent as a user-role message.
