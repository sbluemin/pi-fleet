...
- **Fire-and-forget doctrine**: `carriers_sortie`, `carrier_squadron`, and `carrier_taskforce` only register detached jobs and return `{ job_id, accepted, error? }` immediately. They must not return synchronous result content.
