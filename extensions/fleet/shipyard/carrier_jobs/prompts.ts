...
export const CARRIER_JOBS_MANIFEST: ToolPromptManifest = {
  ...
  description:
    ...
    ` This is not a delegation tool; use it only to inspect summaries, read full archived output once, cancel by job_id, or list jobs.`,
  promptSnippet:
  ...
  whenNotToUse: [
    ...
    `Do not poll carrier_jobs immediately after a carrier launch; terminal results arrive through the [carrier:result] follow-up push.`,
  ],
  usageGuidelines: [
    `carrier_jobs has exactly four actions: status, result, cancel, list.`,
    `After launch, continue independent work or wait for the follow-up push instead of issuing status probes.`,
