...
- **Active-State Guidance**: When querying `status` or `result` (summary) for an `active` job, the response includes an in-band `notice` field wrapped in `<system-reminder>`. This reinforces the doctrine that the Admiral must stop calling tools and wait for the `[carrier:result]` push, which arrives automatically.
...
| `status` | Get the current state and summary of a specific `job_id`. | Includes `<system-reminder>` wrapped `notice` for `active` jobs. |
| `result` | Retrieve the full execution trace and result content (read-once). | `active` jobs return a `retry_after` guide + `notice` (wrapped). |
| `cancel` | Request abortion of an active job. | Includes `notice` (wrapped) if cancellation fails but the job is active. |
