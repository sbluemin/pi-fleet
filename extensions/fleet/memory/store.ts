...
import type { LogEntry, MemoryIndexEntry, MemoryPaths, RawSourceEntry, WikiEntry } from "./types.js";
...
export async function loadIndex(paths: MemoryPaths): Promise<Record<string, MemoryIndexEntry>> {
  try {
    const raw = await readFile(paths.indexFile, "utf8");
    return JSON.parse(raw) as Record<string, MemoryIndexEntry>;
  } catch {
    return {};
  }
}

export async function rebuildIndex(paths: MemoryPaths): Promise<Record<string, MemoryIndexEntry>> {
  await ensureMemoryRoot(paths);
  const entries = await listWiki(paths);
  const nextIndex: Record<string, MemoryIndexEntry> = {};
