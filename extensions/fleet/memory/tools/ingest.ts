...
import {
  MEMORY_INGEST_DESCRIPTION,
  MEMORY_INGEST_MANIFEST,
  buildMemoryIngestSchema,
} from "../prompts.js";
...
export function buildIngestToolConfig() {
  registerToolPromptManifest(MEMORY_INGEST_MANIFEST);

  return {
    name: "memory_ingest",
    label: "Memory Ingest",
    description: MEMORY_INGEST_DESCRIPTION,
    promptSnippet: MEMORY_INGEST_MANIFEST.promptSnippet,
    promptGuidelines: [...MEMORY_INGEST_MANIFEST.usageGuidelines],
    parameters: buildMemoryIngestSchema(),
    async execute(
      _id: string,
      ...
      const patch: Patch = {
        frontmatter: {
          ...
          proposer: String(params.proposer ?? "tool:memory_ingest"),
          created: now,
        },
