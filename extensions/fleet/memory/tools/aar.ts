...
import {
  MEMORY_AAR_DESCRIPTION,
  MEMORY_AAR_MANIFEST,
  buildMemoryAarSchema,
} from "../prompts.js";
...
export function buildAarProposeToolConfig() {
  registerToolPromptManifest(MEMORY_AAR_MANIFEST);

  return {
    name: "memory_aar_propose",
    label: "Memory AAR Propose",
    description: MEMORY_AAR_DESCRIPTION,
    promptSnippet: MEMORY_AAR_MANIFEST.promptSnippet,
    promptGuidelines: [...MEMORY_AAR_MANIFEST.usageGuidelines],
    parameters: buildMemoryAarSchema(),
    async execute(
      _id: string,
      ...
      const patch: Patch = {
        frontmatter: {
          ...
          proposer: String(params.proposer ?? "tool:memory_aar_propose"),
          created: now,
        },
