...
- **Dependency direction**: `admiral/` 라이브러리 모듈 → `core/settings`, `core/keybind`, `core/agentclientprotocol/provider-types` (setter API only), `fleet/shipyard/` (carrier framework, store, tool prompts). 실제 wiring은 `fleet/index.ts`가 담당한다.
