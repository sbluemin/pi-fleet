...
| `prompts.ts` | `PROTOCOL_PREAMBLE` + settings 함수 + `buildAcpSystemPrompt()` (전체 ACP 지침 동적 합성). **Worldview 토글 여부에 따른 PERSONA/TONE 주입 및 `<fleet_role>` 변종(`FLEET_ROLE_PROMPT` ↔ `FLEET_ROLE_PROMPT_NEUTRAL`) 선택 로직을 포함합니다.** |
