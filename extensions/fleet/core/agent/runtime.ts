/**
 * core/agent/runtime.ts — Core 런타임 상태 관리
 *
 ...
 *
 * feature(modes/tools/models/shell)는 sessionStore/configDir을 직접 몰라도 됩니다.
 * core 내부 모듈(executor, orchestrator, panel)은 이 모듈을 통해 접근합니다.
 */
 ...
 *              (e.g. `path.join(extensionDir, "core", ".data")`)
 */
export function initRuntime(dir: string): void {
  dataDir = dir;
  // dataDir이 존재하지 않으면 생성 (core/.data/ 는 런타임에 처음 만들어짐)
 ...
 * core 내부 모듈(executor, panel/state)에서 직접 사용합니다.
 *
