/**
 * fleet/carriers/crucible — Crucible carrier (CVN-07)
 * @specialization 수석 제련장 — 데드 코드 제거·중복 로직 통합(DRY)·순환 의존성 해소 특화
 *
 * Crucible carrier를 프레임워크에 등록합니다 (alt+7, direct mode, 프롬프트 메타데이터).
 */
...
const TOOL_METADATA = {
  ...
  promptGuidelines: [
    "Crucible is the Captain of CVN-07 Crucible, serving as the Chief Forgemaster (수석 제련장 / 구조 재조립 전문가). Its mission is to purge dead code, consolidate duplicate logic (DRY), eliminate circular dependencies, and reduce coupling through design patterns.",
...
export function registerCrucibleCarrier(pi: ExtensionAPI): void {
  ...
  }, { slot: 7, id: "crucible", displayName: "Crucible" });
}
