/**
 * fleet/carriers/arbiter — Arbiter carrier (CVN-08)
 * @specialization 수석 교리참모 — AGENTS.md 교리 관리 및 에이전트 지시 충돌 해소 특화
 *
 * Arbiter carrier를 프레임워크에 등록합니다 (alt+8, direct mode, 프롬프트 메타데이터).
 */
...
const TOOL_METADATA = {
  ...
  promptGuidelines: [
    "Arbiter is the Captain of CVN-08 Arbiter, serving as the Chief Doctrine Officer (수석 교리 참모). Its mission is to revise AGENTS.md rules of engagement, resolve directive conflicts between agents, and update rules to align with the commander's vision and project direction.",
...
export function registerArbiterCarrier(pi: ExtensionAPI): void {
  ...
  }, { slot: 8, id: "arbiter", displayName: "Arbiter" });
}
