...
`packages/fleet-core/src/admiral/bridge` is the host-agnostic bridge layer for Fleet panel, streaming, carrier UI, and renderer-facing data contracts.
...
- **`@sbluemin/fleet-core/admiral/bridge/run-stream`** owns per-run stream state, stream DTOs, and `BridgeStateStorage` injection.
- **`@sbluemin/fleet-core/admiral/bridge/carrier-panel`** owns carrier job/track state plus pure panel view-model builders.
- **`@sbluemin/fleet-core/admiral/bridge/carrier-control`** owns carrier control overlay DTOs and host-port-driven controller logic.
