...
import type { AgentStatus } from "../../_shared/agent-runtime.js";
import type { ColBlock, ColStatus, CollectedStreamData } from "../../_shared/agent-runtime.js";
import { readBridgeState, writeBridgeState } from "./state-store.js";

export type { ColStatus, CollectedStreamData } from "../../_shared/agent-runtime.js";
...
export function appendTextBlock(cli: string, text: string): void {
  ...
  appendTextBlockToRun(run, text);
}

export function appendTextBlockByRunId(runId: string, text: string): void {
...
export function appendThoughtBlock(cli: string, text: string): void {
  ...
  appendThoughtBlockToRun(run, text);
}

export function appendThoughtBlockByRunId(runId: string, text: string): void {
  const run = getRunById(runId);
  if (!run) return;
...
export function upsertToolBlock(
  ...
  upsertToolBlockInRun(run, title, status, toolCallId);
}

export function upsertToolBlockByRunId(
  runId: string,
  title: string,
  status: string,
  toolCallId?: string,
...
export function updateRunStatus(cli: string, agentStatus: AgentStatus): void {
  ...
  updateRunStatusInRun(run, agentStatus);
}

export function updateRunStatusByRunId(runId: string, agentStatus: AgentStatus): void {
  const run = getRunById(runId);
  if (!run) return;
...
export function finalizeRun(
  ...
  finalizeRunByRun(run, finalStatus, options);
}

export function finalizeRunByRunId(
  runId: string,
  finalStatus: "done" | "err",
  options?: {
    sessionId?: string;
...
function appendTextBlockToRun(run: StreamRun, text: string): void {
...
function appendThoughtBlockToRun(run: StreamRun, text: string): void {
  const last = run.blocks[run.blocks.length - 1];
...
function updateRunStatusInRun(run: StreamRun, agentStatus: AgentStatus): void {
...
function finalizeRunByRun(
  ...
  if (!run.thinking.trim() && options?.fallbackThinking) {
    // fallbackThinking은 실시간 순서가 아니라 종료 시점 복구 데이터라 앞쪽에 합성합니다.
