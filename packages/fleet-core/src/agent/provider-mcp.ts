...
export async function startMcpServer(): Promise<string> {
  ...
  return new Promise<string>((resolve, reject) => {
    ...
    srv.listen(0, "127.0.0.1", () => {
      ...
      agentLog("info", `MCP 서버 기동: port=${addr.port}`, { category: "acp" });
      resolve(serverUrl);
    });
    srv.on("error", (err) => {
      agentLog("error", `MCP 서버 오류: ${err.message}`, { category: "acp" });
      reject(err);
    });
  });
}
...
export async function stopMcpServer(): Promise<void> {
  ...
  return new Promise<void>((resolve) => {
    server!.close(() => {
      agentLog("info", "MCP 서버 종료", { category: "acp" });
      server = null;
      serverUrl = null;
      opaquePath = null;
...
export function resolveNextToolCall(
  ...
  if (queue && queue.length > 0) {
    ...
    agentLog("debug", `FIFO resolve: ${pending.toolName} (${toolCallId})`, { category: "acp" });
    ...
    agentLog("debug", `FIFO pre-queue result (${preQueue.length} pending, id=${toolCallId})`, { category: "acp" });
  }
}
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        ...
        Promise.all(promises)
          ...
          .catch((err) => {
            agentLog("error", `MCP 배치 처리 실패: ${(err as Error).message}`, { category: "acp" });
            sendJsonRpcPayload(
              res,
        ...
        processJsonRpc(parsed, token, options)
          ...
          .catch((err) => {
            agentLog("error", `MCP 요청 처리 실패: ${(err as Error).message}`, { category: "acp" });
            if (res.writableEnded) return;
            sendJsonRpcPayload(
              res,
              makeError(null, -32603, "Internal error"),
      ...
      agentLog("error", `MCP 요청 파싱 실패: ${(err as Error).message}`, { category: "acp" });
...
async function processJsonRpc(
  ...
  switch (method) {
    ...
    case "tools/call": {
      ...
      agentLog("debug", `MCP tool call 수신 (FIFO 큐 대기): ${p.name}`, { category: "acp" });
      ...
      if (preQueue && preQueue.length > 0) {
        ...
        if (pendingResult.toolCallId === toolCallId) {
          ...
          agentLog("debug", `FIFO pre-queued 결과 반환: ${p.name} (${toolCallId})`, { category: "acp" });
