...
export const CARRIER_JOBS_MANIFEST: ToolPromptManifest = {
  ...
  description:
    ...
    ` This is not a delegation tool and not a polling tool; use it only to inspect summaries, read full archived output, cancel by job_id, or list jobs.`,
  promptSnippet:
  ...
  whenToUse: [
    ...
    `Use action:"result" with format:"summary" for repeatable summaries. Use format:"full" only after the job is finalized when raw chronological output is needed; full results remain repeatable for 3 hours.`,
  ],
  whenNotToUse: [
    `Do not use carrier_jobs to delegate new work; use carriers_sortie, carrier_squadron, or carrier_taskforce.`,
    `Do not request full results for active jobs. Full archive reads are finalized-only, read-many for 3 hours, and expire by TTL.`,
  ...
  usageGuidelines: [
    ...
    `Full results are finalized-only, read-many for 3h in process memory, and are returned only when format:"full" is explicitly requested.`,
...
export function buildCarrierJobsSchema(): TObject {
  return Type.Object({
    ...
    format: Type.Optional(Type.Unsafe<string>({
      type: "string",
      enum: ["summary", "full"],
      description: "Only used with action=result. Defaults to summary. Full is finalized-only and read-many for 3h.",
    })),
  });
}
