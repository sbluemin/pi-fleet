...
- Fleet domain modules such as `admiral/` (including `_shared/` for detached parallel-job runners, `bridge/` with allowlist exports, `carrier/`, `carrier-jobs/`, `squadron/`, `store/` with isolated `provider-catalog/`, `taskforce/`, and `protocols/` with integrated `standing-orders/`), `admiralty/` (internalized Grand Fleet domain), `services/agent/` (internal physical implementation for normalized agent stream contract, structured into `shared/`, `provider/`, and `dispatcher/`), `services/job/`, unified settings/log/tool-registry services, and `metaphor/`
- Public API contracts and frozen consumer surfaces, including the canonical `@sbluemin/fleet-core/agent/shared/types` stream contract and `public/runtime.ts` for agent runtime assembly
