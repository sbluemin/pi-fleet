...
interface AdapterCarrierFrameworkState extends Omit<CarrierFrameworkState, "modes"> {
  modes: Map<string, { config: CarrierConfig; pi?: ExtensionAPI }>;
}
...
export function registerCarrier(
  ...
  // Carrier 상태 등록
  gs.modes.set(config.id, { config, pi });
...
function getState(): AdapterCarrierFrameworkState {
  let s = (globalThis as any)[CARRIER_FRAMEWORK_KEY] as AdapterCarrierFrameworkState | undefined;
  if (!s) {
    s = {
      modes: new Map(),
      registeredOrder: [],
