...
- **Registration ownership**: Admiral owns prompt/system policy and manifest composition only. Carrier tool registration stays in `packages/fleet-pi-extension/src/fleet/pi-tools.ts`; `packages/fleet-pi-extension/src/fleet/index.ts` only calls the registration API.
