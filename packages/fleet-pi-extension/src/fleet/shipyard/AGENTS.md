...
- **Single Source of Truth**: `store.ts` owns `states.json` (runtime state). `packages/fleet-pi-extension/src/fleet/push-mode-settings.ts` manages the `fleet-push-mode` section in `settings.json` (preferences).
...
- **FIFO guard**: never edit `packages/fleet-core/src/agent/provider-mcp.ts` for carrier job behavior.
