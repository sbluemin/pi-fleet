...
- `grand-fleet/`는 `packages/fleet-pi-extension/src/fleet/**`의 기능을 사용하지 않고 독자적인 `prompts.ts` 조립 솔기(seam)와 `globalThis` 런타임 상태를 통해 동작한다.
...
- `buildFleetAcpSystemPrompt()`는 `packages/fleet-pi-extension/src/fleet/**`를 임포트하지 않고 독자적으로 Fleet ACP 기본 의미와 Grand Fleet Context를 조립하는 **Local Seam** 역할을 한다.
