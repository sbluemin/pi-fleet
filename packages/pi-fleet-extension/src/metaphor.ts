...
const MAX_OPERATION_NAME_LENGTH = 40;
const MAX_OPERATION_INPUT_LENGTH = 200;
...
export function resolveOperationNameModel(
...
export async function generateOperationName(
  ...
  model: Model<Api>,
  userPrompt: string,
  reasoning: OperationReasoningLevel,
): Promise<string | null> {
  try {
    ...
    const preparedPrompt = preparePromptForOperationName(userPrompt);
    if (!preparedPrompt) return null;

    const composed = composeOperationNameRequest({
      worldviewEnabled,
      preparedPrompt: worldviewEnabled
        ? `Generate an operation codename for this request (max ${MAX_OPERATION_NAME_LENGTH} chars total, including the "${OPERATION_PREFIX}" prefix):\n\n${preparedPrompt}`
        : `Generate a short English task summary for this request (max ${MAX_OPERATION_NAME_LENGTH} chars total):\n\n${preparedPrompt}`,
    });
    ...
    const firstLine = sanitizeOperationName(text.split("\n")[0]!.trim());
    if (!firstLine) return null;

    const strippedSummary = stripOperationPrefix(firstLine);
    const finalSummary = worldviewEnabled ? ensureOperationPrefix(firstLine) : strippedSummary || null;
    if (!finalSummary) return null;

    return finalSummary.length > MAX_OPERATION_NAME_LENGTH
      ? finalSummary.slice(0, MAX_OPERATION_NAME_LENGTH - 1) + "…"
      : finalSummary;
  } catch {
    return null;
  }
...
function preparePromptForOperationName(userPrompt: string): string {
  ...
  return sanitized.length > MAX_OPERATION_INPUT_LENGTH
    ? sanitized.slice(0, MAX_OPERATION_INPUT_LENGTH) + "…"
    : sanitized;
}

function sanitizeOperationName(summary: string): string {
  return summary
    .replace(OSC_REGEX, "")
    .replace(ANSI_REGEX, "")
